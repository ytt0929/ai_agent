/**
 * 企业监测 Mock 数据
 * timeRaw 全部用毫秒数（Number），避免 JSON 序列化后丢失 Date 类型
 */

const now = Date.now()

// ════════════════════════════════════════
// 监控任务列表（monitorTasks）
// ════════════════════════════════════════
export const monitorTasks = [
  {
    id: 'MT-001',
    enterpriseName: '明达精工有限公司',
    creditCode: '91330300MA29XXXX1Z',
    source: 'due-diligence',
    sourceLabel: '尽调转入',
    status: 'warning',
    indicators: [
      { id: 'ind-tax', name: '税票波动', condition: '连续下降超过30%', level: 'high', enabled: true },
      { id: 'ind-judicial', name: '司法风险', condition: '新增被执行/诉讼', level: 'high', enabled: true },
    ],
    lastScanAt: '2026-07-01 19:30',
    warningCount: 2,
    latestWarning: { title: '税票连续下降超30%', level: 'high' },
    focused: true,
    createdAt: '2026-06-25',
  },
  {
    id: 'MT-002',
    enterpriseName: '宁波天合新材料有限公司',
    creditCode: '91330200MA2CXXXX8Y',
    source: 'screening',
    sourceLabel: '筛客转入',
    status: 'running',
    indicators: [
      { id: 'ind-industry', name: '工商变更', condition: '法人/股东变更', level: 'medium', enabled: true },
      { id: 'ind-expiry', name: '资料有效期', condition: '过期或即将过期', level: 'medium', enabled: true },
    ],
    lastScanAt: '2026-07-01 18:00',
    warningCount: 0,
    latestWarning: null,
    focused: false,
    createdAt: '2026-06-28',
  },
  {
    id: 'MT-003',
    enterpriseName: '杭州智造装备有限公司',
    creditCode: '91330100MA2BXXXX5T',
    source: 'diagnosis',
    sourceLabel: '风险探查转入',
    status: 'running',
    indicators: [
      { id: 'ind-tax2', name: '税票波动', condition: '连续下降或异常波动', level: 'high', enabled: true },
      { id: 'ind-judicial2', name: '司法风险', condition: '新增被执行', level: 'high', enabled: true },
      { id: 'ind-industry2', name: '工商变更', condition: '法人/股东/经营范围变更', level: 'medium', enabled: true },
    ],
    lastScanAt: '2026-07-01 17:45',
    warningCount: 1,
    latestWarning: { title: '司法风险：新增被执行', level: 'high' },
    focused: false,
    createdAt: '2026-06-20',
  },
  {
    id: 'MT-004',
    enterpriseName: '浙江新源动力有限公司',
    creditCode: '91330400MA1DXXXX3K',
    source: 'manual',
    sourceLabel: '手工新增',
    status: 'paused',
    indicators: [
      { id: 'ind-expiry2', name: '资料有效期', condition: '征信报告/审计报告过期', level: 'medium', enabled: true },
    ],
    lastScanAt: '2026-06-30 10:00',
    warningCount: 1,
    latestWarning: { title: '征信报告已过期', level: 'medium' },
    focused: false,
    createdAt: '2026-06-15',
  },
  {
    id: 'MT-005',
    enterpriseName: '宁波海诚贸易有限公司',
    creditCode: '91330200MA2EXXXX7W',
    source: 'natural-language',
    sourceLabel: '自然语言',
    status: 'running',
    indicators: [
      { id: 'ind-tax3', name: '税票波动', condition: '开票下降超过20%', level: 'high', enabled: true },
      { id: 'ind-judicial3', name: '司法风险', condition: '诉讼新增', level: 'high', enabled: true },
      { id: 'ind-abnormal', name: '经营异常', condition: '经营异常名录新增', level: 'medium', enabled: true },
    ],
    lastScanAt: '2026-07-01 16:20',
    warningCount: 0,
    latestWarning: null,
    focused: false,
    createdAt: '2026-07-01',
  },
  {
    id: 'MT-006',
    enterpriseName: '唐山物桥商贸有限公司',
    creditCode: '91130203MA7EEQ2N0T',
    source: 'due-diligence',
    sourceLabel: '尽调转入',
    status: 'running',
    indicators: [
      { id: 'ind-industry3', name: '工商变更', condition: '法人/股东/经营范围变更', level: 'medium', enabled: true },
      { id: 'ind-tax4', name: '税票波动', condition: '连续下降或异常波动', level: 'high', enabled: true },
      { id: 'ind-judicial4', name: '司法风险', condition: '新增被执行/诉讼', level: 'high', enabled: true },
      { id: 'ind-operation', name: '经营波动', condition: '营收异常波动', level: 'high', enabled: true },
    ],
    lastScanAt: '2026-07-02 08:30',
    warningCount: 3,
    latestWarning: { title: '税负率显著低于行业', level: 'high' },
    focused: true,
    createdAt: '2026-07-02',
  },
]

// ════════════════════════════════════════
// 指标库（indicatorLibrary）
// ════════════════════════════════════════
export const indicatorLibrary = [
  { id: 'ind-tax', name: '税票波动', category: '财务', dataSource: '税票采集系统', defaultCondition: '连续下降超过30%', defaultLevel: 'high', enabled: true },
  { id: 'ind-judicial', name: '司法风险', category: '司法', dataSource: '司法公开数据', defaultCondition: '新增被执行/诉讼', defaultLevel: 'high', enabled: true },
  { id: 'ind-industry', name: '工商变更', category: '工商', dataSource: '工商登记系统', defaultCondition: '法人/股东/经营范围变更', defaultLevel: 'medium', enabled: true },
  { id: 'ind-expiry', name: '资料有效期', category: '资料', dataSource: '内部资料库', defaultCondition: '征信/审计/证照过期或即将过期', defaultLevel: 'medium', enabled: true },
  { id: 'ind-abnormal', name: '经营异常', category: '工商', dataSource: '企业信用信息公示系统', defaultCondition: '列入经营异常名录', defaultLevel: 'medium', enabled: true },
  { id: 'ind-pledge', name: '股权出质', category: '工商', dataSource: '工商登记系统', defaultCondition: '股权出质登记', defaultLevel: 'medium', enabled: true },
  { id: 'ind-penalty', name: '行政处罚', category: '行政', dataSource: '行政处罚公示系统', defaultCondition: '新增行政处罚记录', defaultLevel: 'high', enabled: true },
  { id: 'ind-sentiment', name: '舆情风险', category: '舆情', dataSource: '舆情监测系统', defaultCondition: '负面舆情集中出现', defaultLevel: 'medium', enabled: true },
]

// ════════════════════════════════════════
// 扫描结果（scanResults）
// ════════════════════════════════════════
export const scanResults = [
  {
    taskId: 'MT-001',
    items: [
      { indicatorId: 'ind-tax', indicatorName: '税票波动', status: 'warning', evidence: '近4个月开票金额连续下降，最新月下降超35%，命中阈值。', aiJudgement: '税票连续下降幅度超过监测条件，建议核实企业经营状况，必要时推送尽调。', scannedAt: '2026-07-01 19:30' },
      { indicatorId: 'ind-judicial', indicatorName: '司法风险', status: 'warning', evidence: '新增1条被执行人记录，执行标的500万元。', aiJudgement: '司法风险触发，建议核实被执行原因及对企业偿付能力的影响。', scannedAt: '2026-07-01 19:30' },
    ],
  },
  {
    taskId: 'MT-002',
    items: [
      { indicatorId: 'ind-industry', indicatorName: '工商变更', status: 'normal', evidence: '近30天内无工商变更。', aiJudgement: '工商状态正常，无重要变更。', scannedAt: '2026-07-01 18:00' },
      { indicatorId: 'ind-expiry', indicatorName: '资料有效期', status: 'normal', evidence: '所有资料均在有效期内。', aiJudgement: '资料有效期正常。', scannedAt: '2026-07-01 18:00' },
    ],
  },
  {
    taskId: 'MT-003',
    items: [
      { indicatorId: 'ind-tax2', indicatorName: '税票波动', status: 'normal', evidence: '开票数据正常，无连续下降。', aiJudgement: '税票状态正常。', scannedAt: '2026-07-01 17:45' },
      { indicatorId: 'ind-judicial2', indicatorName: '司法风险', status: 'warning', evidence: '新增1条被执行案件，执行法院为杭州市中级人民法院，执行标的300万元。', aiJudgement: '司法风险触发，建议关注。', scannedAt: '2026-07-01 17:45' },
      { indicatorId: 'ind-industry2', indicatorName: '工商变更', status: 'normal', evidence: '近30天无重要变更。', aiJudgement: '工商状态正常。', scannedAt: '2026-07-01 17:45' },
    ],
  },
  {
    taskId: 'MT-004',
    items: [
      { indicatorId: 'ind-expiry2', indicatorName: '资料有效期', status: 'warning', evidence: '征信报告已过有效期5天。', aiJudgement: '征信报告过期，建议尽快更新。', scannedAt: '2026-06-30 10:00' },
    ],
  },
  {
    taskId: 'MT-005',
    items: [
      { indicatorId: 'ind-tax3', indicatorName: '税票波动', status: 'normal', evidence: '开票数据正常。', aiJudgement: '无需处置。', scannedAt: '2026-07-01 16:20' },
      { indicatorId: 'ind-judicial3', indicatorName: '司法风险', status: 'normal', evidence: '无新增司法案件。', aiJudgement: '司法状态正常。', scannedAt: '2026-07-01 16:20' },
      { indicatorId: 'ind-abnormal', indicatorName: '经营异常', status: 'normal', evidence: '未列入经营异常名录。', aiJudgement: '经营异常状态正常。', scannedAt: '2026-07-01 16:20' },
    ],
  },
]

// ════════════════════════════════════════
// 预警列表（monitorWarnings）
// ════════════════════════════════════════
export const monitorWarnings = [
  {
    id: 'MW-001',
    taskId: 'MT-001',
    enterpriseName: '明达精工有限公司',
    indicatorId: 'ind-tax',
    indicatorName: '税票波动',
    level: 'high',
    title: '税票连续下降超过30%',
    triggerCondition: '连续下降超过30%',
    triggerEvidence: '近4个月开票金额连续下降，最新月下降超35%，低于同行业均值。',
    aiJudgement: '税票连续下降幅度超过监测条件，建议核实企业经营状况，必要时推送尽调。',
    suggestion: '推送到智能尽调并生成重点核查项，确认下降原因及对企业偿付能力的影响。',
    pushedToDueDiligence: false,
    focused: true,
    handled: false,
    actionLogs: [{ title: '已加入重点关注', time: '2026-07-01 19:35' }],
    createdAt: '2026-07-01 19:30',
  },
  {
    id: 'MW-002',
    taskId: 'MT-001',
    enterpriseName: '明达精工有限公司',
    indicatorId: 'ind-judicial',
    indicatorName: '司法风险',
    level: 'high',
    title: '司法风险：新增被执行',
    triggerCondition: '新增被执行/诉讼',
    triggerEvidence: '新增1条被执行人记录，执行法院宁波市中级人民法院，执行标的500万元。',
    aiJudgement: '司法风险触发，建议核实被执行原因及对企业偿付能力的影响。',
    suggestion: '建议同步税票、司法证据到尽调任务，由AI生成风险核查清单。',
    pushedToDueDiligence: false,
    focused: false,
    handled: false,
    actionLogs: [],
    createdAt: '2026-07-01 19:30',
  },
  {
    id: 'MW-003',
    taskId: 'MT-003',
    enterpriseName: '杭州智造装备有限公司',
    indicatorId: 'ind-judicial2',
    indicatorName: '司法风险',
    level: 'high',
    title: '司法风险：新增被执行',
    triggerCondition: '新增被执行',
    triggerEvidence: '新增1条被执行案件，执行法院杭州市中级人民法院，执行标的300万元。',
    aiJudgement: '司法风险触发，建议关注。',
    suggestion: '核实被执行原因，判断是否影响企业信用状况。',
    pushedToDueDiligence: false,
    focused: false,
    handled: false,
    actionLogs: [],
    createdAt: '2026-07-01 17:45',
  },
  {
    id: 'MW-004',
    taskId: 'MT-004',
    enterpriseName: '浙江新源动力有限公司',
    indicatorId: 'ind-expiry2',
    indicatorName: '资料有效期',
    level: 'medium',
    title: '征信报告已过期',
    triggerCondition: '征信报告/审计报告过期',
    triggerEvidence: '征信报告已过有效期5天。',
    aiJudgement: '征信报告过期，建议尽快更新。',
    suggestion: '联系企业补充最新征信报告。',
    pushedToDueDiligence: false,
    focused: false,
    handled: false,
    actionLogs: [],
    createdAt: '2026-06-30 10:00',
  },
  {
    id: 'MW-005',
    taskId: 'MT-006',
    enterpriseName: '唐山物桥商贸有限公司',
    indicatorId: 'ind-tax4',
    indicatorName: '税票波动',
    level: 'high',
    title: '税负率显著低于行业',
    triggerCondition: '增值税税负率低于行业均值50%',
    triggerEvidence: '增值税税负率0.8%，仅为行业均值2.8%的29%。',
    aiJudgement: '税负率显著低于行业平均水平，可能存在低报收入或虚增成本的风险，建议重点核查。',
    suggestion: '推送到智能尽调并生成重点核查项，核实税负结构及收入真实性。',
    pushedToDueDiligence: false,
    focused: true,
    handled: false,
    actionLogs: [{ title: '已加入重点关注', time: '2026-07-02 08:35' }],
    createdAt: '2026-07-02 08:30',
  },
  {
    id: 'MW-006',
    taskId: 'MT-006',
    enterpriseName: '唐山物桥商贸有限公司',
    indicatorId: 'ind-tax4',
    indicatorName: '税票波动',
    level: 'medium',
    title: '开票收入与申报收入不一致',
    triggerCondition: '开票收入与申报收入差异超过3%',
    triggerEvidence: '开票收入2275.98万元，申报收入2175.46万元，差异4.4%。',
    aiJudgement: '开票收入与申报收入存在显著差异，可能影响收入真实性判断。',
    suggestion: '核实差异原因，确认是否存在未申报收入。',
    pushedToDueDiligence: false,
    focused: false,
    handled: false,
    actionLogs: [],
    createdAt: '2026-07-02 08:30',
  },
  {
    id: 'MW-007',
    taskId: 'MT-006',
    enterpriseName: '唐山物桥商贸有限公司',
    indicatorId: 'ind-operation',
    indicatorName: '经营波动',
    level: 'medium',
    title: '购销两头在外',
    triggerCondition: '主要供应商和客户均位于外地',
    triggerEvidence: '主要供应商和客户均位于外地，存在两头在外经营模式。',
    aiJudgement: '两头在外模式增加业务真实性核查难度，建议加强上下游核实。',
    suggestion: '核实主要供应商和客户的真实性及关联关系。',
    pushedToDueDiligence: false,
    focused: false,
    handled: false,
    actionLogs: [],
    createdAt: '2026-07-02 08:30',
  },
]

// ════════════════════════════════════════
// 旧数据保留兼容（筛客 addWatchedCompany 等）
// ════════════════════════════════════════

export const warnings = [
  {
    id: 'W001',
    type: 'enterprise',
    level: 'high',
    title: '税票连续下降',
    enterprise: { name: '浙江XX制造有限公司', creditCode: '91330100MA2KJ8X26R' },
    ruleName: '杭州智造装备-税票/司法监控',
    summary: '本月销项发票下降62%（-1240万元），低于同行业波动均值（-15%）',
    time: '10分钟前',
    timeRaw: now - 10 * 60000,
    read: false,
    triggerReason: '本月增值税销项发票金额较上月下降62%（-1240万元），低于同行业波动均值（-15%）',
    trendData: [
      { month: '3月', value: 1950 },
      { month: '4月', value: 2000 },
      { month: '5月', value: 1200 },
      { month: '6月', value: 760, abnormal: true },
    ],
    industryAvg: '下降15%',
    impactAssessment: [
      '销项发票连续3个月下降，趋势未逆转',
      '远低于行业均值，异常程度较高',
      '近6个月无新增诉讼，工商状态正常',
      '综合判断：经营端承压可能性较大',
    ],
    historyWarnings: [
      { date: '05-15', title: '经营范围变更', level: 'low', read: true },
      { date: '03-20', title: '法人变更', level: 'low', read: true },
    ],
  },
  {
    id: 'W002',
    type: 'enterprise',
    level: 'high',
    title: '新增被执行',
    enterprise: { name: 'XX科技有限公司', creditCode: '91310000MA1FL5XQ9X' },
    ruleName: '司法监控',
    summary: '宁波市中级人民法院执行案件，执行金额500万',
    time: '2小时前',
    timeRaw: now - 2 * 3600000,
    read: false,
    triggerReason: '该企业新增1条被执行人记录，执行法院为宁波市中级人民法院，执行标的500万元',
    trendData: [],
    industryAvg: '',
    impactAssessment: [
      '被执行金额500万，占注册资本的10%',
      '近30天内新增第2条执行记录',
      '法人同时为另一家被执行企业的股东',
      '综合判断：信用风险显著上升',
    ],
    historyWarnings: [
      { date: '06-10', title: '股权出质', level: 'medium', read: true },
    ],
  },
  {
    id: 'W003',
    type: 'compliance',
    level: 'high',
    title: '征信报告已过期',
    enterprise: { name: '浙江XX制造有限公司', creditCode: '91330100MA2KJ8X26R' },
    docType: '征信报告',
    summary: '最后更新：2025-05-15，有效期30天',
    expireDate: '2025-06-14',
    daysOverdue: 3,
    time: '过期3天',
    timeRaw: now - 3 * 24 * 3600000,
    read: false,
  },
  {
    id: 'W004',
    type: 'enterprise',
    level: 'medium',
    title: '法人变更',
    enterprise: { name: 'YY贸易有限公司', creditCode: '91330300MA2L0X9012' },
    ruleName: '法人变更监控',
    summary: '法人从张三变更为李四',
    time: '昨天',
    timeRaw: now - 24 * 3600000,
    read: true,
    triggerReason: '企业法人发生变更，由张三变更为李四，变更日期2026-06-26',
    trendData: [],
    industryAvg: '',
    impactAssessment: [
      '法人变更属正常工商行为',
      '新法人李四无失信或被执行记录',
      '企业工商状态正常，无其他异常',
      '综合判断：风险较低，建议关注后续经营变化',
    ],
    historyWarnings: [],
  },
  {
    id: 'W005',
    type: 'compliance',
    level: 'medium',
    title: '尽调报告即将过期',
    enterprise: { name: 'XX科技有限公司', creditCode: '91310000MA1FL5XQ9X' },
    docType: '尽调报告',
    summary: '最后更新：2025-04-20，有效期90天，还剩5天',
    expireDate: '2026-07-02',
    daysRemaining: 5,
    time: '还剩5天',
    timeRaw: now + 5 * 24 * 3600000,
    read: true,
  },
  {
    id: 'W006',
    type: 'enterprise',
    level: 'low',
    title: '经营范围变更',
    enterprise: { name: 'ZZ贸易有限公司', creditCode: '91330600MA2M0X3456' },
    ruleName: '工商变更监控',
    summary: '经营范围新增进出口业务',
    time: '3天前',
    timeRaw: now - 3 * 24 * 3600000,
    read: true,
    triggerReason: '企业经营范围发生变更，新增进出口业务',
    trendData: [],
    industryAvg: '',
    impactAssessment: [
      '经营范围变更属正常行为',
      '新增进出口业务符合企业发展方向',
      '无其他异常信号',
      '综合判断：正常变更，无需特别关注',
    ],
    historyWarnings: [],
  },
  {
    id: 'W007',
    type: 'compliance',
    level: 'medium',
    title: '审计报告即将到期',
    enterprise: { name: 'AA集团有限公司', creditCode: '91330000MA2H0X7890' },
    docType: '审计报告',
    summary: '最后更新：2025-07-01，有效期365天，还剩22天',
    expireDate: '2026-07-19',
    daysRemaining: 22,
    time: '还剩22天',
    timeRaw: now + 22 * 24 * 3600000,
    read: true,
  },
]

export const rules = [
  {
    id: 'R001',
    name: '杭州智造装备-税票/司法监控',
    enterprises: ['浙江XX制造有限公司'],
    dimensions: [
      { name: '税票波动', condition: '连续3个月下降超30%', level: 'high' },
      { name: '司法风险', condition: '新增被执行', level: 'high' },
    ],
    notifyMethod: '站内消息',
    status: 'running',
    createdAt: '2026-06-20',
    triggerCount: 2,
    lastTrigger: '今天 14:32',
    history: [
      { time: '06-27 14:32', title: '税票连续下降', warningId: 'W001' },
      { time: '06-22 09:15', title: '新增被执行', warningId: 'W002' },
    ],
  },
  {
    id: 'R002',
    name: 'XX科技-法人变更监控',
    enterprises: ['XX科技有限公司'],
    dimensions: [
      { name: '法人变更', condition: '任意变更', level: 'medium' },
      { name: '股东变更', condition: '任意变更', level: 'medium' },
    ],
    notifyMethod: '站内消息',
    status: 'running',
    createdAt: '2026-06-15',
    triggerCount: 0,
    lastTrigger: '无',
    history: [],
  },
  {
    id: 'R003',
    name: 'YY贸易-经营范围变更监控',
    enterprises: ['YY贸易有限公司'],
    dimensions: [
      { name: '经营范围变更', condition: '任意变更', level: 'low' },
    ],
    notifyMethod: '站内消息',
    status: 'running',
    createdAt: '2026-06-10',
    triggerCount: 1,
    lastTrigger: '3天前',
    history: [
      { time: '06-24 10:00', title: '经营范围变更', warningId: 'W006' },
    ],
  },
  {
    id: 'R004',
    name: 'ZZ制造-经营指标监控',
    enterprises: ['ZZ制造有限公司'],
    dimensions: [
      { name: '用电量异常', condition: '月用电量下降超40%', level: 'medium' },
      { name: '社保断缴', condition: '社保缴纳人数下降', level: 'medium' },
    ],
    notifyMethod: '站内消息',
    status: 'paused',
    createdAt: '2026-05-20',
    triggerCount: 5,
    pausedAt: '06-20',
    history: [],
  },
]
